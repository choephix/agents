 ...
 * Mirrors the harness layout one level over: `~/.omp/agent/shadow-agent-sessions/<encoded-cwd>`,
...
export function shadowSessionDir(cwd: string): string {
	...
	return path.join(agentDir, "shadow-agent-sessions", path.basename(projectDir));
}
